...
//            checkCoinState();
    ...
    protected static Coin[] findUnspentCoins(Stream<Coin> unspents, UInt256 asset_id, Fixed8 amount) {
        ...
        if (sum.compareTo(amount) < 0)
            throw new RuntimeException(DataFormat.getErrorDesc4NoBalance(String.format("insuficient balance, sum=%s, amount=%s", sum, amount)));
    ...
    public <T extends Transaction> T makeTransaction(T tx, Fixed8 fee, UInt160 from) throws CoinException {
        ...
        if (pay_coins.values().stream().anyMatch(p -> p == null))
            throw new CoinException(DataFormat.getErrorDesc4NoBalance("insuficient balance"));
    ...
    static Block from(String ss) {
        try {
            return Serializable.from(neo.Helper.hexToBytes(ss), Block.class);
    ...
    public static long byteToLong(List<Integer> bytes) {
        long value = 0;
        for (int i = bytes.size() - 1; i >= 0; i--) {
            value = (value * 256) + bytes.get(i);
        }
        return value;
    }

    public static List<Integer> hexStringToBytes(String hexString) {
        List<Integer> hes = new ArrayList<>();
        for (int i = 0; i < hexString.length(); i = i + 2) {
            int dec_num = Integer.parseInt(hexString.substring(i, i + 2), 16);
            hes.add(dec_num);
        }
        return hes;
    }
